...
// Repo: multilang-python | Env: dev
// Stack: python / flask / pip
...
if [ -f pyproject.toml ]; then pip3 install poetry && poetry install; fi
        ...
        stage('Run Tests') {
            steps {
                sh '''
                . venv/bin/activate && python3 -m pytest --tb=short
                '''
            }
        }
                      ...
                      withSonarQubeEnv('SonarQube') {
                          sh """
                              . venv/bin/activate && ${scannerHome}/bin/sonar-scanner \
                                  ...
                                  -Dsonar.python.version=3 -Dsonar.sources=. -Dsonar.exclusions=venv/**,**/__pycache__/**,**/*.pyc \
